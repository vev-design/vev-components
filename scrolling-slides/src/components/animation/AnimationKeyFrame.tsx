...
export function KeyFrame({
  ...
  return (
    <SilkeBox column gap="s">
      ...
      />
      <ScaleField value={frame.scale} onChange={(scale) => handleAttrChange('scale', scale)} />

      <RotateFiled value={frame.rotate} onChange={(rotate) => handleAttrChange('rotate', rotate)} />
      ...
      <SilkeBox gap="s">
        ...
        <SilkeColorPickerButton
          ...
          onChange={(backgroundColor) => handleAttrChange('backgroundColor', backgroundColor)}
        />
      </SilkeBox>
    </SilkeBox>
  );
