...
interface LiquidEtherWebGL {
  ...
  updatePalette?: (paletteTexture: THREE.DataTexture) => void;
}

const defaultColors = ['#5227FF', '#FF9FFC', '#B19EEF'];

// Move shader code outside component to avoid recreation
const face_vert = `
...
function LiquidEthersection({
  ...
  useEffect(() => {
    ...
    class Output {
      ...
      camera: THREE.Camera;
      output: THREE.Mesh;
      constructor(paletteTexture: THREE.DataTexture) {
      ...
      updatePalette(paletteTexture: THREE.DataTexture) {
        const uniforms = (this.output.material as THREE.RawShaderMaterial).uniforms;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any, paletteTexture: THREE.DataTexture) {
        this.props = props;
      ...
      init(paletteTexture: THREE.DataTexture) {
        if (!Common.renderer) return;
        this.props.$wrapper.prepend(Common.renderer.domElement);
        this.output = new Output(paletteTexture);
      }
      updateProps(props: any) {
        this.props = { ...this.props, ...props };
        if (this.autoDriver) {
          if (typeof props.autoDemo !== 'undefined') this.autoDriver.enabled = props.autoDemo;
          if (typeof props.autoSpeed !== 'undefined') this.autoDriver.speed = props.autoSpeed;
          if (typeof props.autoResumeDelay !== 'undefined') this.autoDriver.resumeDelay = props.autoResumeDelay;
          if (typeof props.autoRampDuration !== 'undefined') this.autoDriver.rampDurationMs = props.autoRampDuration * 1000;
        }
        if (typeof props.autoIntensity !== 'undefined') Mouse.autoIntensity = props.autoIntensity;
        if (typeof props.takeoverDuration !== 'undefined') Mouse.takeoverDuration = props.takeoverDuration;
      }
      updatePalette(paletteTexture: THREE.DataTexture) {
        if (this.output) {
          this.output.updatePalette(paletteTexture);
        }
      }
      resize() {
        Common.resize();
  ...
  useEffect(() => {
    ...
    if (colorsChanged) {
      const newPaletteTex = makePaletteTexture(colorsArray);
      webgl.updatePalette(newPaletteTex);
      prevColorsRef.current = colorsArray;
    }
  }, [colorsArray]);

  return <div ref={mountRef} className={[styles.wrapper, className || ''].join(' ')} style={style} />;
}
