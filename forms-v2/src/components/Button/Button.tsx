...
import { registerVevComponent, useDispatchVevEvent, useGlobalStateRef, useModel } from '@vev/react';
...
function Button({ ...props }: Props) {
  ...
  const formState = props.variables.reduce((res, curr) => {
    return {
      ...res,
      [curr?.variable]: store?.current?.variables.find((v) => v.key === curr?.variable)?.value,
    };
  }, {});
